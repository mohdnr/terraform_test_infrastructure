...
resource "aws_iam_role_policy_attachment" "clamav_policies" {
  role       = aws_iam_role.task_execution_role.name
  policy_arn = aws_iam_policy.clamav_policies.arn
}

data "aws_iam_policy_document" "clamav_policies" {
  statement {
...
resource "aws_iam_policy" "clamav_policies" {
  name   = "ClamAVTaskExecutionPolicies"
  path   = "/"
  policy = data.aws_iam_policy_document.clamav_policies.json
}
