...
class ReportController(Controller):
    ...
    def show(self, request: Request, view: View):
        ...
        reports = builder.simple_paginate(5, page)
    ...
    def save(self, request: Request, response: Response):
        ...
        if csp_report and all (key in csp_report for key in ('document-uri','referrer','violated-directive','original-policy','blocked-uri')):
          domain = urlparse(csp_report['document-uri']).netloc
          report = Report.create(id=str(uuid4()), domain=domain, document_uri=csp_report['document-uri'], referrer=csp_report['referrer'], violated_directive=csp_report['violated-directive'], original_policy=csp_report['original-policy'], blocked_uri=csp_report['blocked-uri'])
